...
void render_frame(std::vector<hkpRigidBody*> bodies,std::vector<DxModel*> models);
void cleanD3D(std::vector<DxModel*> models);
                   ...
                   int nCmdShow)
{
    ...
    hWnd = CreateWindowEx(NULL, L"WindowClass", L"3D Bowling Deluxe",
	...
	//models.push_back(new DxModel(d3ddev,L"BowlingLane.X",D3DXVECTOR3(0,0,0),50.0f));
	...
	models.push_back(new DxModel(d3ddev,L"BowlingBall.X",D3DXVECTOR3(5.0f,10.0f,500.0f),10.0f));
	...
	for(int row=4;row>0;row--)
	{
		for(int pin=1;pin<=row;pin++)
		{
			...
			models.push_back(new DxModel(d3ddev,L"BowlingPin.X",D3DXVECTOR3(pinPos(0),pinPos(1),pinPos(2)),0.1f));
		}
	}
    ...
    while(TRUE)
    {
		...
		for(unsigned int i=0;i<bodies.size();i++)
		{
			...
			models[i]->modelRotation = D3DXQUATERNION((float)rot(0),(float)rot(1),(float)rot(2),(float)rot(3));

			hkVector4 pos = bodies[i]->getPosition();
			models[i]->position = D3DXVECTOR3((float)pos(0),(float)pos(1),(float)pos(2));
		}
		for(unsigned int i=0;i<models.size();i++)
			models[i]->Update();

		//Do Rendering here
        render_frame(bodies,models);
    }
...
void render_frame(std::vector<hkpRigidBody*> bodies,std::vector<DxModel*> models)
{
    ...
    D3DXMatrixLookAtLH(&matView,
		&D3DXVECTOR3 (0.0f, 100.0f, 500.0f),    // the camera position
		&D3DXVECTOR3 (0.0f, 0.0f, 10.0f),    // the look-at position
