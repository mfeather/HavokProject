...
private:
    ...
    float rotationAngle;			// an ever-increasing float value
...
public:
	...
	void Update()
	{
		...
		D3DXMatrixRotationY(&matRotateY, rotationAngle);    // the rotation matrix

	};
	void Draw(LPDIRECT3DDEVICE9 d3ddev)
	{
		D3DXMATRIX matWorld;							   // a matrix to store the total of all transforms
		matWorld = matScale * matRotateY * matTranslate;   // multiply all the transforms and store in world matrix
